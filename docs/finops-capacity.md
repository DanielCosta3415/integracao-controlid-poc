...
| Baixa | `docs/reports` acumular relatórios históricos grandes | Reports versionados são docs de auditoria | Manter apenas reports sem dados reais; revisar tamanho periodicamente. |
...
| Alta | RTO/RPO e retenção ainda dependem de `ops.local.json` e validação humana | Preencher `rtoRpo.*`, executar backup/restore-smoke e aprovar política; ver `docs/residual-risk-closure.md`. |
