...
> **Referência** · Público: desenvolvimento e operação · Responsável: Engenharia · Última validação: 2026-08-12.

Esta é a fonte canônica para o comportamento operacional do SQLite: arquivos
criados em runtime, WAL, concorrência, inicialização, migrações pendentes e
comandos seguros. O inventário de entidades, índices, retenção, backup e
restauração permanece em
[Modelo de dados, integridade e recuperação](data-model-and-recovery.md).

O projeto usa SQLite local por padrão. A cadeia de conexão
`DefaultConnection` fica em `appsettings.json` e aponta para
...
- `MonitorEvents` e `PushCommands` podem conter payloads pessoais/sensíveis e devem seguir a política de retenção em [docs/seguranca-privacidade/privacy-and-data-retention.md](../seguranca-privacidade/privacy-and-data-retention.md).
...
O teste smoke inicia o stub local, executa fluxos HTTP e grava relatórios em
`artifacts/`. Uma evidência só entra no [histórico](../historico/relatorios/README.md)
após sanitização e revisão humana.
...
O mapa completo de tabelas, índices, riscos de integridade e procedimento de restauração está em [docs/dados/data-model-and-recovery.md](data-model-and-recovery.md).
